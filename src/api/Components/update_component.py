...
class UpdateComponent:
    ...
    def update_user(p_id_usuario, p_usuario, p_universidad, p_contrasena_actual, p_nueva_contrasena=None):
        try:
            ...
            sql_verify_password = """
                SELECT contrasena FROM Projectg2.Usuarios
                WHERE id_usuario = %s AND estado = true
            """
            record_verify = (p_id_usuario,)
            verify_result = DataBaseHandle.getRecords(sql_verify_password, 1, record_verify)

            if not verify_result['result']:
                message = 'Error al verificar la contraseña'
                return internal_response(result, data, message)

            # Asegúrate de que verify_result['data'] sea un diccionario
            if not verify_result['data']:
                message = 'Usuario no encontrado o contraseña incorrecta'
                return internal_response(result, data, message)

            current_password = verify_result['data'].get('contrasena')
            if current_password != p_contrasena_actual:
                message = 'La contraseña actual es incorrecta'
                return internal_response(result, data, message)

            # Construir la consulta de actualización dinámica
            sql_update = "UPDATE Projectg2.Usuarios SET "
            updates = []
            params = []
